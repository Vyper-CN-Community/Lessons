# Vyper Contract Interaction Challenge: Workshop
...
This structure groups a person's favorite number and their name.

### The add_person Function Explained

The primary function we'll use for this challenge is `add_person`. Here's how it works:
...
3.  It creates a new `Person` struct instance using the provided name and favorite number.
...
If the contract starts with `index = 1` (because index 0 is already filled with "Mark"), we need the `add_person` function to execute **when `index` is 3** to store our desired data at `list_of_people[3]`.

To achieve this, we need to call `add_person` multiple times:

1.  **Call 1 (Already done)**: Adds `(7, "Mark")` at `index = 0`. Index becomes 1.
...
*   **Mappings**: Vyper mappings provide a flexible way to store key-value data. They don't have a fixed size but allow accessing values via their keys.
*   **Structs**: Custom data types (`Person`) allow grouping related information.
*   **Index Management**: Using a dedicated state variable (`index`) to control the insertion position, simulating array-like sequential addition.
*   **Gas Costs**: Every state-changing transaction consumes gas on a live blockchain network.
