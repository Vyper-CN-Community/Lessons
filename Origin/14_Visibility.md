...
### 1. internal (Default)
If you declare a state variable without any specific visibility keyword, like our `my_favorite_number: uint256` example above, it defaults to **internal**. This means the variable can only be accessed from within the contract itself or by contracts that inherit from it. External accounts or other independent contracts cannot directly read its value without a dedicated function created for that purpose.
...
### 2. public
To make a state variable readable from outside the contract, you need to declare it as **public**. You do this by wrapping the variable's type with `public()`:
...
my_favorite_number: uint256 # Internal by default
```
...
my_favorite_number: public(uint256) # Now public!
```
