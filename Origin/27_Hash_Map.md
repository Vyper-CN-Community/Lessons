...
1.  **Inconvenience**: Users or other contracts need to know the correct numerical index for the specific entry they want.
...
A key characteristic of HashMaps in Vyper is how they handle lookups for keys that haven't been explicitly assigned a value. Instead of causing an error, they return the **default value** for the specified `ValueType`. Common default values include:
...
1.  **Deployment**: Immediately after deployment, the `name_to_favorite_number` mapping is empty (conceptually). If you query it with the key "Mariah", it returns `0` (default value).
2.  **Adding Data**: Call `add_person` with `name = "Mariah"` and `favorite_number = 30`. This executes `self.name_to_favorite_number["Mariah"] = 30`.
3.  **Retrieving Data**: If you now query the mapping using the key "Mariah", it will return `30`.

**The Benefit:** We can now directly retrieve Mariah's favorite number using her name as the identifier, without needing to know her position (index) in any list.
...
*   Declare HashMaps using the syntax `HashMap[KeyType, ValueType]`.
...
*   `HashMap` and `List` are fundamental data structures in Vyper. Choosing between them involves considering access patterns (direct lookup vs. iteration) and gas costs.
