...
- **@pure**: This decorator signifies that a function promises **not** to read from or modify the contract's state storage. Functions marked `@pure` (along with `@view` functions) can often be called off-chain without incurring gas costs, provided the call itself doesn't trigger a state-changing transaction.
...
result: uint256 = 5 // 2  # Result is 2
```

Using a single slash `/` is not the standard way to perform integer division in Vyper. Always use `//` when dividing integers to ensure predictable, floored results.
...
3. **todo**: Shows how `pass` can be used to create a valid, empty function body.
...
- Calling `return_many()` returns the values: `uint256: 123` and `bool: true`.
